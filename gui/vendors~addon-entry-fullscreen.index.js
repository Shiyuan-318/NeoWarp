...
/* harmony default export */ __webpack_exports__["default"] = (async function (_ref) {
  ...
  function updateBrowserFullscreen() {
    if (addon.settings.get("browserFullscreen") && !addon.self.disabled) {
      ...
      } else if (!addon.tab.redux.state.scratchGui.mode.isFullScreen && document.fullscreenElement !== null && !isExitingFullscreen) {
        isExitingFullscreen = true;
        // NeoWarp: Use requestAnimationFrame to prevent stutter when exiting fullscreen
  ...
  async function initScaler() {
    ...
    try {
      ...
      // Monitor scaler not found
      return;
    }
    const updateScale = () => {
      const stage = document.querySelector('[class*="stage-wrapper_full-screen"] [class*="stage_stage"]');
      if (!stage) return;
